...
class DynamicShapesDataset:
    def __init__(self, length=64, seed=42, batch_size=8):
        ...
        self.xs = [
            np.random.normal(size=(s,)).astype(np.float32)
    ...
    class RegressionPreTrainedModelWithGradientCheckpointing(PreTrainedModel):
        ...
        def __init__(self, config):
            ...
            self.layers = nn.ModuleList(
                [nn.Linear(config.hidden_size, config.hidden_size) for _ in range(4)]
            )
    ...
    def get_regression_trainer(
        ...
        **kwargs,
        ...
        args = RegressionTrainingArguments(
            output_dir, a=a, b=b, keep_report_to=keep_report_to, **kwargs
        )
        return Trainer(
            model,
            args,
...
class TrainerIntegrationCommon:
        ...
        if is_pretrained:
            ...
            if not safe_weights:
                ...
                state_dict = safetensors.torch.load_file(
                    os.path.join(checkpoint, SAFE_WEIGHTS_NAME)
                )
    ...
    def convert_to_sharded_checkpoint(self, folder, save_safe=True, load_safe=True):
        ...
        shard_files = [
            shard_name.replace(
                f".{extension}", f"-{idx+1:05d}-of-{len(keys):05d}.{extension}"
            )
...
class TrainerIntegrationPrerunTest(TestCasePlus, TrainerIntegrationCommon):
    ...
    def test_gradient_checkpointing(self):
        ...
        previous_params = {
            k: v.detach().clone() for k, v in trainer.model.named_parameters()
        }
    ...
    def test_reduce_lr_on_plateau_args(self):
        ...
        lr_scheduler = torch.optim.lr_scheduler.ReduceLROnPlateau(
            optimizer, factor=0.2, patience=5, cooldown=2
        )
        trainer = Trainer(
            model,
            args,
...
class TrainerIntegrationTest(TestCasePlus, TrainerIntegrationCommon):
    ...
    def test_number_of_steps_in_training_with_ipex(self):
        for mix_bf16 in [True, False]:
            # Regular training has n_epochs * len(train_dl) steps
            trainer = get_regression_trainer(
                learning_rate=0.1, use_ipex=True, bf16=mix_bf16, use_cpu=True
            )
            train_output = trainer.train()
            self.assertEqual(
    ...
    def test_multiple_peft_adapters(self):
        ...
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            trainer = Trainer(
                tiny_model, args, tokenizer=tokenizer, train_dataset=train_dataset
            )
    ...
    def test_get_eval_dataloader_without_persistent_workers(self):
        ...
        tiny_gpt2 = GPT2LMHeadModel(config)
        args = TrainingArguments(
            "./test", report_to="none", dataloader_persistent_workers=False
        )

        # Single evaluation dataset
        eval_dataset = RegressionDataset()
        trainer = Trainer(
            tiny_gpt2, args, train_dataset=train_dataset, eval_dataset=eval_dataset
        )
    ...
    def test_lomo(self):
        config = LlamaConfig(
            vocab_size=100, hidden_size=32, num_hidden_layers=3, num_attention_heads=4
        )
        ...
        with tempfile.TemporaryDirectory() as tmpdir:
            # Trainer without inf/nan filter
            args = TrainingArguments(
                tmpdir, learning_rate=1e-2, logging_steps=5, optim="lomo", max_steps=20
            )
        ...
        for name, param in tiny_llama.named_parameters():
            self.assertFalse(
                torch.allclose(
                    param,
                    previous_params[name].to(param.device),
                    rtol=1e-12,
                    atol=1e-12,
                )
            )
    ...
    def test_galore_matched_modules(self):
        ...
        for expected_value, module_name in zip(expected_values, module_names):
            is_module_matched, is_regex = check_target_module_exists(
                regex_patterns, module_name, return_is_regex=True
            )
        ...
        for expected_value, module_name in zip(expected_values, module_names):
            is_module_matched, is_regex = check_target_module_exists(
                exact_patterns, module_name, return_is_regex=True
            )
            self.assertTrue(is_module_matched == expected_value)
            if is_module_matched:
        ...
        for expected_value, module_name in zip(expected_values, module_names):
            is_module_matched, is_regex = check_target_module_exists(
                simple_regex, module_name, return_is_regex=True
            )
            self.assertTrue(is_module_matched == expected_value)
            if is_module_matched:
                self.assertTrue(is_regex)
    ...
    def test_galore_lr_display_without_scheduler(self):
        ...
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            self.assertEqual(
                trainer.get_learning_rates(), [learning_rate, learning_rate]
            )
    ...
    def test_galore_lr_display_with_scheduler(self):
        ...
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            self.assertTrue(
                logs[num_warmup_steps - 2]["learning_rate"] == learning_rate
            )
    ...
    def test_predict_with_ipex(self):
        for mix_bf16 in [True, False]:
            trainer = get_regression_trainer(
                a=1.5, b=2.5, use_ipex=True, bf16=mix_bf16, use_cpu=True
            )
            preds = trainer.predict(trainer.eval_dataset).predictions
            x = trainer.eval_dataset.x
            self.assertTrue(np.allclose(preds, 1.5 * x + 2.5))
            ...
            # With a number of elements not a round multiple of the batch size
            trainer = get_regression_trainer(
                a=1.5, b=2.5, eval_len=66, use_ipex=True, bf16=mix_bf16, use_cpu=True
            )
            preds = trainer.predict(trainer.eval_dataset).predictions
            x = trainer.eval_dataset.x
            self.assertTrue(np.allclose(preds, 1.5 * x + 2.5))
    ...
    def test_dynamic_shapes(self):
        ...
        args = TrainingArguments(
            "./regression", eval_accumulation_steps=2, report_to="none"
        )
        trainer = Trainer(model, args, eval_dataset=eval_dataset)
    ...
    def test_safe_checkpoints(self):
        for save_safetensors in [True, False]:
            with tempfile.TemporaryDirectory() as tmpdir:
                trainer = get_regression_trainer(
                    output_dir=tmpdir, save_steps=5, save_safetensors=save_safetensors
                )
                trainer.train()
                self.check_saved_checkpoints(
                    tmpdir,
                    5,
                    int(self.n_epochs * 64 / self.batch_size),
                    safe_weights=save_safetensors,
                )
    ...
    def test_load_best_model_with_save(self):
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            assert os.path.exists(
                os.path.join(tmpdir, "checkpoint-9")
        ...
        for initial_safe in [False, True]:
            for loaded_safe in [False, True]:
                with tempfile.TemporaryDirectory() as tmpdir:
                    ...
                    checkpoint = os.path.join(tmpdir, "checkpoint-5")
                    self.convert_to_sharded_checkpoint(
                        checkpoint, load_safe=initial_safe, save_safe=loaded_safe
                    )
    ...
    def test_trainer_eval_mrpc(self):
        ...
        training_args = TrainingArguments(
            output_dir="./examples", use_cpu=True, report_to="none"
        )
    ...
    def test_data_map_callback(self):
        pass
    ...
    def test_flos_extraction(self):
        ...
        def assert_flos_extraction(trainer, wrapped_model_to_check):
            self.assertEqual(
                trainer.model, trainer.accelerator.unwrap_model(wrapped_model_to_check)
            )
            self.assertGreaterEqual(
                getattr(
                    trainer.accelerator.unwrap_model(wrapped_model_to_check).config,
                    "total_flos",
                    0,
                ),
                0,
    ...
    @unittest.skip(
        reason="torch 2.0.0 gives `ModuleNotFoundError: No module named 'torchdynamo'`."
    )
    @require_torch_non_multi_gpu
    @require_torchdynamo
    ...
    def test_accelerator_config_empty(self):
        ...
        with tempfile.TemporaryDirectory() as tmp_dir:
            ...
            if GRAD_ACCUM_KWARGS_VERSION_AVAILABLE:
                ...
                self.assertNotIn(
                    "sync_each_batch", trainer.accelerator.gradient_state.plugin_kwargs
                )
    ...
    def test_accelerator_config_from_yaml(self):
        ...
        with tempfile.TemporaryDirectory() as tmp_dir:
            ...
            # Leaves all options as something *not* basic
            args = RegressionTrainingArguments(
                output_dir=tmp_dir, accelerator_config=path_file
            )
            trainer = Trainer(model=model, args=args, eval_dataset=eval_dataset)
            self.assertEqual(trainer.accelerator.split_batches, True)
            self.assertEqual(trainer.accelerator.dispatch_batches, True)
    ...
    def test_accelerate_config_from_dataclass_grad_accum(self):
        ...
        with tempfile.TemporaryDirectory() as tmp_dir:
            ...
            trainer = Trainer(model=model, args=args, eval_dataset=eval_dataset)
            self.assertEqual(
                trainer.accelerator.gradient_state.plugin_kwargs["num_steps"], 10
            )
            self.assertEqual(
                trainer.accelerator.gradient_state.plugin_kwargs["adjust_scheduler"],
                False,
            )
            self.assertEqual(
                trainer.accelerator.gradient_state.plugin_kwargs[
    ...
    def test_accelerator_custom_state(self):
        ...
        with tempfile.TemporaryDirectory() as tmp_dir:
            with self.assertRaises(ValueError) as cm:
                ...
                self.assertIn(
                    "Please define this beforehand", str(cm.warnings[0].message)
                )
            _ = Accelerator()
            _ = RegressionTrainingArguments(
                output_dir=tmp_dir, accelerator_config={"use_configured_state": True}
            )
        AcceleratorState._reset_state(reset_partial_state=True)
    ...
    def test_torch_dtype_to_json(self):
        ...
        for dtype in [
            ...
            with tempfile.TemporaryDirectory() as tmp_dir:
                args = TorchDtypeTrainingArguments(
                    output_dir=tmp_dir, torch_dtype=torch_dtype
                )
    ...
    def test_eval_use_gather_object(self):
        ...
        model = RegressionDictModel()
        args = TrainingArguments(
            "./regression", report_to="none", eval_use_gather_object=True
        )
        trainer = Trainer(
            model, args, train_dataset=train_dataset, eval_dataset=eval_dataset
        )
...
if is_torch_available():
    ...
    if is_bitsandbytes_available():
        ...
        optim_test_params.append(
            (
                TrainingArguments(
                    optim=OptimizerNames.PAGED_ADAMW_8BIT, output_dir="None"
                ),
                bnb.optim.AdamW,
                default_adam_kwargs,
            )
        ...
        optim_test_params.append(
            (
                TrainingArguments(
                    optim=OptimizerNames.PAGED_LION_8BIT, output_dir="None"
                ),
                bnb.optim.Lion,
                default_lion_kwargs,
            )
    ...
    if is_torchdistx_available():
        ...
        optim_test_params.append(
            (
                TrainingArguments(
                    optim=OptimizerNames.ADAMW_ANYPRECISION, output_dir="None"
                ),
